...
GitHub Task Sync workflow 已接入 Issue / PR / workflow_run 事件，可通过 CHAO_SYNC_DATABASE_URL 将 GitHub 链接回写 task；
GitHub Task Sync 失败 Issue 回流已接入，配置同步数据库后失败会创建 / 追加跟进 Issue；
...
```text
官方 MCP SDK 客户端联调；
为 GitHub Task Sync 补充 Console 可见性增强；
...
```text
1. 当前已有本地 Tool Gateway 拦截入口、首批真实 handler、MCP stdio 外壳和 MCP SDK 依赖声明，但尚未完成真实 MCP 客户端联调；
...
```text
1. 完成真实 MCP 客户端联调；
2. 为 GitHub Task Sync 补充 Console 可见性增强；
