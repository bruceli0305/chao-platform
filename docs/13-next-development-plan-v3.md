...
```text
1. I22：Web Console 增加任务搜索 / 状态过滤 / 等级过滤，降低任务定位成本；
2. I23：Web Console 补齐空态、错误态和数据库不可用时的可读提示；
3. H7：Agent Runner 增加真实分支创建 / 检查策略，不只生成 branch plan；
4. H8：Agent Runner 引入隔离工作区或 Docker Sandbox 的最小执行流；
5. E7：将现有 permission policy 下沉到 MCP Server / 工具协议层拦截；
6. C8：为 context_chunks 增加 embedding 写入与向量相似度搜索。
```
