...
H8. 引入隔离工作区或 Docker Sandbox 最小执行流。
```
...
```text
1. H8：Agent Runner 引入隔离工作区或 Docker Sandbox 的最小执行流；
2. E7：将现有 permission policy 下沉到 MCP Server / 工具协议层拦截；
3. C8：为 context_chunks 增加 embedding 写入与向量相似度搜索。
```
