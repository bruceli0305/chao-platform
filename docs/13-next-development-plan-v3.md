...
```text
1. I23：Web Console 补齐空态、错误态和数据库不可用时的可读提示；
2. H7：Agent Runner 增加真实分支创建 / 检查策略，不只生成 branch plan；
3. H8：Agent Runner 引入隔离工作区或 Docker Sandbox 的最小执行流；
4. E7：将现有 permission policy 下沉到 MCP Server / 工具协议层拦截；
5. C8：为 context_chunks 增加 embedding 写入与向量相似度搜索。
```
