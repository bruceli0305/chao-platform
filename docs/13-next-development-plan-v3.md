...
D7. 生成 skill_usage 与 skill_execution_plan，记录 Skill hash、默认 gates 与合并 gates；
D8. 刑部验证优先使用 skill_execution_plan.combined_gates。
```
...
D5 app/chao/skill_runtime.py 已在执行前读取 required_skill_paths 对应的 SKILL.md，并仅记录 headings、hash 和长度，不持久化正文。
D6 GitHub PR 模板已增加 Skill 字段和 SKILL.md 阅读确认项。
D7 skill_execution_plan 已接入 route_result、Markdown 史官记录、CLI console-task 和 Web Console 任务详情。
D8 app/chao/nodes/xingbu.py 已优先使用 skill_execution_plan.combined_gates 生成 validation_result。
```
...
E12. 接入 LLM Chat 最小 client 与 tool_calls 审计。
E13. 使用官方 MCP Python SDK 完成本地客户端 smoke 联调。
```
...
E14 app/chao/llm_policy.py 已接入 LLM 外发策略，--execute 仅允许 L1/L2、D0/D1 和已登记 provider/model；L3/L4 必须显式开启 governed egress 且存在未过期授权记录。
E15 db/migrations/010_llm_egress_authorizations.sql 已接入独立有效期授权记录，authorize-llm-egress 会写入授权、事件和工具审计。
E16 Console 已接入 llm_egress_authorizations 展示与过期授权风险视图。
E17 audit-llm-egress-authorizations 已接入过期授权审计，可 dry-run 检测并可用 --apply 标记 EXPIRED，同时写入事件和工具审计。
E18 .github/workflows/llm-egress-audit.yml 已接入 schedule / workflow_dispatch，可使用 CHAO_AUDIT_DATABASE_URL 对外部审计库执行过期授权审计。
E19 LLM egress 授权过期审计已接入 GitHub Issue 回流，失败或发现过期授权时会创建 / 追加跟进 Issue。
E20 .github/workflows/github-task-sync.yml 已接入 Issue / PR / workflow_run 事件，可使用 CHAO_SYNC_DATABASE_URL 将 GitHub 链接回写 task。
E21 GitHub Task Sync 失败 Issue 回流已接入，配置同步数据库后失败会创建 / 追加跟进 Issue。
E22 mcp-sdk-smoke 命令已使用官方 MCP Python SDK 启动本地 mcp-serve，完成 initialize、tools/list，并支持可选 tools/call smoke。
```
...
I13 Web Console 风险明细已接入：根路径展示阻塞任务、Runner 失败、失败 gate、工具风险、pending/stale tool calls 和 GitHub 风险。
...
I24 GitHub Sync 面板已接入未绑定交付任务可见性：CLI / Web Console 均可展示已 DELIVERED 但缺少 github_links 的任务。
...
```text
1. 将 Skill Execution Plan 继续下沉到 Runner / Sandbox 自动执行入口；
2. 完善 GitHub Issue / PR / Commit / CI 与 task 的双向同步；
3. Web Console 增加审批写操作与产品化 UI。
```
