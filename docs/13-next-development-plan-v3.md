...
```text
1. H7：Agent Runner 增加真实分支创建 / 检查策略，不只生成 branch plan；
2. H8：Agent Runner 引入隔离工作区或 Docker Sandbox 的最小执行流；
3. E7：将现有 permission policy 下沉到 MCP Server / 工具协议层拦截；
4. C8：为 context_chunks 增加 embedding 写入与向量相似度搜索。
```
