...
def test_runner_workspace_dry_run_records_event_and_tool_call(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda workspace_plan, dry_run=True, **_kwargs: {
...
def test_runner_workspace_apply_records_created_event(monkeypatch):
    ...
    def fake_create_runner_workspace(workspace_plan, dry_run=True, **_kwargs):
...
def test_runner_workspace_uses_explicit_repository_config(monkeypatch):
    calls = {
        "workspace_plan": None,
    ...
    repository = _repository_config(
        ...
        sandbox_root="/opt/chao/sandboxes/server-repo",
        branch_prefix="codex/",
    )

    def fake_create_runner_workspace(workspace_plan, **kwargs):
        calls["workspace_plan"] = workspace_plan
    ...
    result = CliRunner().invoke(
        cli.app,
        ["runner-workspace", "TASK-1", "--repository", "server-repo"],
    )

    assert result.exit_code == 0
    assert calls["workspace_plan"]["base_ref"] == "origin/main"
    assert calls["workspace_plan"]["workspace_path"].startswith("/opt/chao/sandboxes/server-repo/")
    assert calls["workspace_kwargs"]["repo_root"] == "/opt/chao/workspaces/server-repo"
    assert calls["workspace_kwargs"]["allowed_workspace_root"] == (
        "/opt/chao/sandboxes/server-repo"
    )
